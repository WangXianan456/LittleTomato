...
import { characters, cartoonIds } from "./characters";
...
export default function SettingsPanel() {
  ...
  const [collection, setCollection] = useState<"all" | "original" | "cartoon">("all");
      ...
      <div className="settings-body">
        ...
        <div ref={content} className="settings-content">
          ...
          <fieldset disabled={!saved || busy}>
            {tab === "characters" && <section aria-labelledby="characters-heading"><div className="section-heading"><span className="collection-label">MEET YOUR LITTLE FRIENDS</span><h2 id="characters-heading">今天，想和谁一起？</h2><p>十五位小伙伴，同一份陪伴。选好后点击保存。</p></div><div className="collection-filter" aria-label="角色分类">{([{ id: "all", label: "全部 · 15" }, { id: "original", label: "原创伙伴 · 5" }, { id: "cartoon", label: "卡通伙伴 · 10" }] as const).map(item => <button key={item.id} type="button" aria-pressed={collection === item.id} onClick={() => setCollection(item.id)}>{item.label}</button>)}</div><div className="character-grid" role="group" aria-label="选择桌宠角色">{characters.filter(item => collection === "all" || cartoonIds.includes(item.id) === (collection === "cartoon")).map(item => <button key={item.id} type="button" className="character-card" aria-pressed={draft.character === item.id} aria-label={item.name} onClick={() => update("character", item.id)}><span className="character-art reduce-motion" data-palette={item.id}><span className="tomato" data-character={item.id}><PetFeatures character={item.id} /></span></span><span className="character-name">{item.name}<span className="character-check" aria-hidden="true">{draft.character === item.id ? "✓" : "+"}</span></span><span className="character-subtitle">{item.subtitle}</span></button>)}</div><p className="character-hint">切换伙伴会保留你的计时与专注记录。</p></section>}
            ...
            {tab === "appearance" && <section aria-labelledby="appearance-heading"><div className="section-heading"><h2 id="appearance-heading">外观与陪伴</h2><p>安静一点，或靠近一点，都由你决定。</p></div>
              <div className="settings-card"><div className="size-setting"><label htmlFor="pet-size"><strong>桌宠大小</strong><span>{draft.petSize} px</span></label><input id="pet-size" name="petSize" type="range" min="80" max="240" step="10" value={draft.petSize} onChange={e => update("petSize", Number(e.target.value))} /><div className="range-labels"><span>小巧 · 80</span><span>默认 · 160</span><span>醒目 · 240</span></div><div className={`pet-preview ${draft.reducedMotion ? "reduce-motion" : ""}`} aria-label="桌宠大小预览"><div className="tomato" data-character={draft.character} style={{ zoom: Number(draft.petSize) / 240 }}><PetFeatures character={draft.character} /></div><small>大小预览 · 保存后应用到桌面</small></div></div>{toggle("alwaysOnTop", "始终置顶", "让小番茄留在其他普通窗口前面")}{toggle("reducedMotion", "减少动画", "停止呼吸、眨眼和叶片摆动；也遵循系统偏好")}</div>
